...
def get_pp_tickets():
    try:
        ...
        params = []
        if is_admin:
            # Admin: all completed tickets, all time, no assignee filter
            query = """
                SELECT t.*, u.username as assignee_name, u2.username as creator_name,
                       p.project_name, u3.username as collaborator_name
                FROM trueday.tickets t
                LEFT JOIN trueday.users u ON t.assignee_id = u.id
                LEFT JOIN trueday.users u2 ON t.creator_id = u2.id
                LEFT JOIN trueday.users u3 ON t.collaborator_id = u3.id
                LEFT JOIN trueday.project p ON t.project_id = p.project_id
                WHERE UPPER(t.status) = 'COMPLETED'
            """
        else:
            # Regular user: only their own tickets completed today
            query = """
                SELECT t.*, u.username as assignee_name, u2.username as creator_name,
                       p.project_name, u3.username as collaborator_name
                FROM trueday.tickets t
