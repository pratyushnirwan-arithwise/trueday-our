...
# ⚠️ FIXED: Extended from 15 minutes to 24 hours
    ...
    try:
        ...
        try:
            ...
            if due_date and due_date.strip():
                ...
                if ticket_data:
                    creation_date_str = ticket_data[0]  # created_at
        ...
        current_status = current_ticket[5]  # status is at index 5
        ...
        if new_status and new_status.upper() == 'COMPLETED':
            ticket_approver_id = current_ticket[9]  # approver_id is at index 9
        ...
        status_changed = str(current_ticket[5]) != str(data.get('status'))
        if status_changed:
            old_status = current_ticket[5]
            new_status = data.get('status')
        ...
        assignee_changed = str(current_ticket[1]) != str(data.get('assignee_id'))
        if assignee_changed:
            old_assignee_id = current_ticket[1]
        ...
        collaborator_changed = str(current_ticket[8]) != str(data.get('collaborator_id'))
        if collaborator_changed:
            old_collaborator_id = current_ticket[8]
        ...
        approver_changed = str(current_ticket[9]) != str(data.get('approver_id'))
        if approver_changed:
            old_approver_id = current_ticket[9]
        ...
        project_changed = str(current_ticket[7]) != str(data.get('project_id'))
        if project_changed:
            old_project_id = current_ticket[7]
        ...
        priority_changed = str(current_ticket[4]) != str(data.get('priority'))
        if priority_changed:
            old_priority = current_ticket[4]
        ...
        due_date_changed = normalize_date_str(current_ticket[6]) != normalize_date_str(data.get('due_date'))
        if due_date_changed:
            old_due = normalize_date_str(current_ticket[6])
...
"""
                ...
                if collaborator_id:
                    ...
                    if collaborator_row and collaborator_row[1]:
                        collaborator_name = collaborator_row[0] or "there"
                        collaborator_email = collaborator_row[1]
...
def get_user(user_id):
    ...
    if user:
        # Fetch assigned project IDs from project_users table
        assigned_projects = []
        try:
            conn2 = get_db_connection()
            cursor2 = conn2.cursor()
            cursor2.execute(
                "SELECT project_id FROM trueday.project_users WHERE user_id = %s",
                (user_id,)
            )
            project_rows = cursor2.fetchall()
            for row in project_rows:
                # Handle both dict and tuple rows
                p_id = row.get('project_id') if isinstance(row, dict) else row[0]
        ...
        user_id_val = user.get('id') if isinstance(user, dict) else user[0]
        ...
        phone = user.get('phone') if isinstance(user, dict) else user[4]
        location = user.get('location') if isinstance(user, dict) else user[5]
        department = user.get('department') if isinstance(user, dict) else user[6]
        join_date = user.get('join_date') if isinstance(user, dict) else user[7]
...
def get_employee_name():
    try:
        ...
        # Query to get all users with their assigned project IDs
        query = """
            SELECT u.id, u.username, array_agg(pu.project_id) as project_ids
            FROM trueday.users u
...
def permanently_delete_ticket(ticket_id):
    try:
        ...
        cursor.execute("DELETE FROM trueday.ticket_history WHERE ticket_id = %s", (ticket_id,))
        # Delete the ticket
...
def upload_attachment():
    ...
    try:
        ...
        uploader_name = uploader_row[0] if uploader_row else 'Unknown User'
...
# @app.route('/delete_ticket_message/<int:message_id>', methods=['DELETE', 'POST', 'OPTIONS'])
...
#         return response
#     try:
#         # Support both JSON body and query params for user_id
#         data = request.get_json(silent=True) or {}
#         user_id = data.get('user_id') or request.args.get('user_id')

#         if not all([message_id, user_id]):
#             return jsonify({"error": "message_id and user_id are required"}), 400

#         conn = get_db_connection()
#         cursor = conn.cursor()

#         # Ensure ownership and fetch message for response
#         cursor.execute("SELECT ticket_id, user_id, message_text FROM trueday.messages WHERE id = %s", (message_id,))
#         row = cursor.fetchone()
#         if not row:
#             cursor.close(); conn.close()
...
#         ticket_id, owner_user_id, message_text = row
#         if str(owner_user_id) != str(user_id):
#             cursor.close(); conn.close()
#             return jsonify({"error": "Not authorized to delete this message"}), 403

#         cursor.execute("DELETE FROM trueday.messages WHERE id = %s", (message_id,))
#         conn.commit()
#         cursor.close(); conn.close()
...
# @app.route('/api/delete_ticket_message/<int:message_id>', methods=['DELETE', 'POST', 'OPTIONS'])
# def api_delete_ticket_message(message_id):
#     return delete_ticket_message(message_id)
...
def get_ticket_attachments(ticket_id):
    ...
    try:
        ...
        # Map indexes to fields matching the SELECT above
        attachment_list = [{
            "id": att[0],                 # a.id
            "file_name": att[2],          # a.file_name
            "file_path": att[3],          # a.file_path (relative filename stored)
            "file_type": att[4],          # a.file_type
            "file_size": att[5],          # a.file_size
            "uploaded_at": att[6].isoformat() if att[6] else None,  # a.uploaded_at
            "uploaded_by": att[7]         # username
...
def validate_jwt_endpoint():
    ...
    try:
        ...
        cur.execute("SELECT id, username, email FROM trueday.users WHERE id = %s", (user_id,))
...
def serve_react_app(path):
    ...
    if path.startswith('api/') or path.startswith('assets/') or path.startswith('uploads/'):
        print("[DEBUG] Path is API/static/upload, returning 404.")
...
# Remove all other catch-all routes and add this single, robust catch-all at the very end of the file
@app.route('/', defaults={'path': ''})
@app.route('/<path:path>')
def catch_all(path):
    print(f"[DEBUG] Catch-all route triggered for path: '{path}'")
...
def patched_update_ticket_status(ticket_id):
    ...
    try:
        ...
        if new_status == 'COMPLETED':
            assignee = assignee_id or updated_ticket[1]
            if assignee:
                cursor.execute(
...
def send_progress_pulse_reminders():
    ...
    for ticket_id, ticket_title, user_id, username, email in tickets:
