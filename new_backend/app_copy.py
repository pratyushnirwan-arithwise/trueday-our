...
<div style="font-family:Segoe UI, sans-serif; font-size:14px; color:#333; line-height:1.6; max-width:600px; border:1px solid #e0e0e0; border-radius:8px; padding:20px; background-color:#fafafa;">
 ...
 <p style="color: #555; font-size:13px;">
    Best regards,<br />
   <img src="https://ariths.com/td/trueday_logpng"style="vertical-align:middle; height:5px; width:15px; margin-right:5px;"  />
<br />
...
def create_tables():
    ...
    try:
        ...
        if cursor.fetchone()[0] == 0:
            cursor.execute('''
                INSERT INTO trueday.statuses (status_name, is_default, is_active, display_order) VALUES
                ('New', TRUE, TRUE, 1),
                ('In Progress', FALSE, TRUE, 2),
                ('QA', FALSE, TRUE, 3),
                ('Resolved', FALSE, TRUE, 4),
                ('On Hold', FALSE, TRUE, 5)
            ''')
...
@app.route('/<path:path>')
def serve(path):
    # 🔒 Block access if no cookie is present
...
def add_timeline_comment():
    try:
        ...
        # Resolve user_id from username (fallback to None)
        cursor.execute(
...
# 4. GET /api/tickets  ─── Filtered ticket list
@app.route('/api/tickets', methods=['GET'])
def get_filtered_tickets():
    try:
        ...
        if project:
            conditions.append("p.project_name ILIKE %s")
            params.append(f'%{project}%')

        if status:
            conditions.append("t.status ILIKE %s")
        ...
        """, params)
        tickets = cur.fetchall()
        return jsonify([dict(r) for r in tickets])
    except Exception as e:
...
def get_pp_history(ticket_id):
    try:
        ...
        cursor = conn.cursor(row_factory=dict_row)
        cursor.execute("""
            SELECT id, ticket_id, work_done, challenge, learning,
...
def add_progresspulse_entry():
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM trueday.progresspulse WHERE ticket_id = %s", (data.get('ticket_id'),))
        cursor.execute("""
