...
import { Search, Check, Save, MessageSquare, FolderOpen, Activity, Users, ExternalLink } from "lucide-react";
...
export default function ProgressPulse() {
  ...
  // Admin can edit only their own tickets; read-only on others'
  const currentUserId = currentUser?.id ? String(currentUser.id) : localStorage.getItem('userId');
  const isOwnTicket = active.assignee_id ? String(active.assignee_id) === currentUserId : true;
                    ...
                    >
                      ...
                      <div className="pp-ticket-item-bottom-row">
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                          <div className="pp-ticket-item-avatar" style={{ background: avColor(item.assignee_name) }} title={item.assignee_name}>
                            {avInit(item.assignee_name)}
                          </div>
                          {item.collaborator_name && (
                            <div
                              className="pp-ticket-item-avatar"
                              style={{
                                background: avColor(item.collaborator_name),
                                width: '22px',
                                height: '22px',
                                fontSize: '9px',
                              ...
                              title={`Collaborator: ${item.collaborator_name}`}
                            >
                              {avInit(item.collaborator_name)}
                            </div>
                          )}
                        </div>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/dashboard?openTicket=${item.ticket_id}`);
                          }}
                          title="Go to ticket on Kanban board"
                          style={{
                            background: 'transparent',
                            border: 'none',
                            color: activeId === item.ticket_id ? '#6d117e' : '#64748b',
                            cursor: 'pointer',
                            padding: '4px',
                            borderRadius: '4px',
                            display: 'flex',
                          ...
                          onMouseEnter={(e) => {
                            e.currentTarget.style.background = 'rgba(109, 17, 126, 0.08)';
                            e.currentTarget.style.color = '#6d117e';
                          }}
                          onMouseLeave={(e) => {
                            e.currentTarget.style.background = 'transparent';
                            e.currentTarget.style.color = activeId === item.ticket_id ? '#6d117e' : '#64748b';
                          }}
                        >
        ...
        <div className="pp-main-area pp-scroll">
          ...
          <div className="pp-main-header">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', flex: 1, minWidth: 0 }}>
              ...
              {isAdmin && activeId && (active.assignee_name || active.collaborator_name) && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', paddingLeft: '4px' }}>
                  {active.assignee_name && (
                    <span style={{
                      display: 'inline-flex', alignItems: 'center', gap: '6px',
                      background: avColor(active.assignee_name) + '22',
                      border: `1.5px solid ${avColor(active.assignee_name)}55`,
                      borderRadius: '20px', padding: '3px 10px 3px 4px',
                      fontSize: '12px', fontWeight: 600, color: avColor(active.assignee_name),
                      lineHeight: 1
                    }}>
                      <span style={{
                        width: 20, height: 20, borderRadius: '50%',
                        background: avColor(active.assignee_name),
                        color: '#fff', display: 'inline-flex',
                        alignItems: 'center', justifyContent: 'center',
                        fontSize: '10px', fontWeight: 700, flexShrink: 0,
                        lineHeight: 1
                      }}>{avInit(active.assignee_name)}</span>
                      <span style={{ lineHeight: 1 }}>{active.assignee_name}</span>
                      <span style={{ fontSize: '10px', opacity: 0.65, fontWeight: 400, lineHeight: 1, alignSelf: 'center' }}>Assignee</span>
                    </span>
                  )}
                  {active.collaborator_name && (
                    <span style={{
                      display: 'inline-flex', alignItems: 'center', gap: '6px',
                      background: avColor(active.collaborator_name) + '22',
                      border: `1.5px solid ${avColor(active.collaborator_name)}55`,
                      borderRadius: '20px', padding: '3px 10px 3px 4px',
                      fontSize: '12px', fontWeight: 600, color: avColor(active.collaborator_name),
                      lineHeight: 1
                    }}>
                      <span style={{
                        width: 20, height: 20, borderRadius: '50%',
                        background: avColor(active.collaborator_name),
                        color: '#fff', display: 'inline-flex',
                        alignItems: 'center', justifyContent: 'center',
                        fontSize: '10px', fontWeight: 700, flexShrink: 0,
                        lineHeight: 1
                      }}>{avInit(active.collaborator_name)}</span>
                      <span style={{ lineHeight: 1 }}>{active.collaborator_name}</span>
                      <span style={{ fontSize: '10px', opacity: 0.65, fontWeight: 400, lineHeight: 1, alignSelf: 'center' }}>Collaborator</span>
                    </span>
                  )}
                </div>
              )}
                      ...
                      {[
                        ...
                        <div key={index} className="pp-entry-field-group">
                          ...
                          <textarea
                            rows={4}
                            value={card.value || ''}
                            onChange={(e) => canEdit ? setDraft({ ...draft, [card.key]: e.target.value }) : undefined}
                            readOnly={!canEdit}
                            className="pp-entry-field-textarea"
                            placeholder={card.placeholder}
                            style={!canEdit ? { cursor: 'default', opacity: 0.75, background: 'var(--bg-subtle, #f8fafc)' } : {}}
                          />
                        </div>
                      ))}
                    </div>
