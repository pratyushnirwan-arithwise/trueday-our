...
const DashBoard = () => {
  ...
  // Auto-open ticket when navigated from Progress Pulse via ?openTicket=ID
  useEffect(() => {
    if (!tickets.length) return;
    const params = new URLSearchParams(window.location.hash.split('?')[1] || '');
    const openTicketId = params.get('openTicket');
    if (openTicketId) {
      const match = tickets.find(t => String(t.ticket_id) === openTicketId || String(t.id) === openTicketId);
      if (match) {
        setEditingTicket(match);
